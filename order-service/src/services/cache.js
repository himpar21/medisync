const DEFAULT_TTL_SECONDS = Math.max(5, Number(process.env.ORDER_CACHE_TTL_SECONDS || 30));
const CACHE_NAMESPACE = String(process.env.ORDER_CACHE_NAMESPACE || "order:cache:").trim();
...
async function bootstrapRedis() {
  ...
  try {
    ...
    redisClient.on("error", (error) => {
      if (redisEnabled) {
        console.warn("Order cache Redis disconnected, falling back to memory:", error.message);
      }
      redisEnabled = false;
    });
    ...
    redisEnabled = true;
    console.log("Order cache connected to Redis");
  } catch (error) {
    redisEnabled = false;
    redisClient = null;
    console.warn("Order cache using memory fallback:", error.message);
  }
}
