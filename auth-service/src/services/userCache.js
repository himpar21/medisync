const DEFAULT_TTL_SECONDS = Math.max(5, Number(process.env.AUTH_CACHE_TTL_SECONDS || 45));
const CACHE_NAMESPACE = String(process.env.AUTH_CACHE_NAMESPACE || "auth:cache:").trim();
...
async function bootstrapRedis() {
  ...
  try {
    ...
    redisClient.on("error", (error) => {
      if (redisEnabled) {
        console.warn("Auth cache Redis disconnected, falling back to memory:", error.message);
      }
      redisEnabled = false;
    });
    ...
    redisEnabled = true;
    console.log("Auth cache connected to Redis");
  } catch (error) {
    redisEnabled = false;
    redisClient = null;
    console.warn("Auth cache using memory fallback:", error.message);
  }
}
