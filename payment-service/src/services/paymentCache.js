const DEFAULT_TTL_SECONDS = Math.max(5, Number(process.env.PAYMENT_CACHE_TTL_SECONDS || 30));
const CACHE_NAMESPACE = String(process.env.PAYMENT_CACHE_NAMESPACE || "payment:cache:").trim();
...
async function bootstrapRedis() {
  ...
  try {
    ...
    redisClient.on("error", (error) => {
      if (redisEnabled) {
        console.warn("Payment cache Redis disconnected, falling back to memory:", error.message);
      }
      redisEnabled = false;
    });
    ...
    redisEnabled = true;
    console.log("Payment cache connected to Redis");
  } catch (error) {
    redisEnabled = false;
    redisClient = null;
    console.warn("Payment cache using memory fallback:", error.message);
  }
}
