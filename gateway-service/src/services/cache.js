const DEFAULT_TTL_SECONDS = Math.max(5, Number(process.env.GATEWAY_CACHE_TTL_SECONDS || 15));
const CACHE_NAMESPACE = String(process.env.GATEWAY_CACHE_NAMESPACE || "gateway:cache:").trim();
...
async function bootstrapRedis() {
  ...
  try {
    ...
    redisClient.on("error", (error) => {
      if (redisEnabled) {
        console.warn("Gateway cache Redis disconnected, falling back to memory:", error.message);
      }
      redisEnabled = false;
    });
    ...
    redisEnabled = true;
    console.log("Gateway cache connected to Redis");
  } catch (error) {
    redisEnabled = false;
    redisClient = null;
    console.warn("Gateway cache using memory fallback:", error.message);
  }
}
